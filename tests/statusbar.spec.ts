/**
 * Tests for the StatusBar's pure helper: the column-aware model-name fitter.
 * The StatusBar itself mounts through Ink and is covered by a manual smoke
 * (`pnpm dsh --profile tui` in a real TTY); here we lock the arithmetic that
 * decides what its columns say. The token accumulator behind `in:` / `out:`
 * moved to `usage.spec.ts` when `/context` needed the same numbers.
 */
