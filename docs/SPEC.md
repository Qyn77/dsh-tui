...
**Spacing and indent normalization.** Paragraphs render with one line of vertical breathing room (`marginY=1`) above and below, so model-written song lyrics and dialog don't look smushed against surrounding blocks. Inside a paragraph, every leading space and tab at the start of each newline-continued line is stripped — this counteracts the model's habit of hand-indenting continuation lines by 10+ spaces, which the terminal would otherwise show as a right-shifted run-on. The CommonMark 4-space code-block rule does not apply here: marked keeps deep-indent continuations as paragraph text rather than promoting them to code blocks. Consecutive newlines (`\n\n`) are preserved so blank lines stay blank. Spaces at the very start of the text, and spaces between inline elements (`**bold**` and the next word), are preserved. The strip runs at the parse boundary (pre-`marked.lexer`), so blank-line-separated indented blocks — which CommonMark would otherwise promote to a `╭─╮` code frame — also flatten back into normal paragraphs.
