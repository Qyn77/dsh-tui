...
- **Unselected rows carry a leading space to match the selected row's background block.** The highlight is an inverted `cyan` block that pads itself by one column on each side; without the matching space the `·` column would step sideways as the selection moved.
- **The name never shrinks; the description truncates with an ellipsis.** They are two flex children of one row, and Yoga's answer to a row that does not fit is to take columns off both — which is how `/copy` came out as `/cop` on a 90-column terminal while its description ran to the edge. `flexShrink={0}` on the name and `wrap="truncate-end"` on the description make the loss land where it costs the least. Truncating rather than wrapping is also what keeps the windowing above honest: a row that wrapped would make the palette taller than `paletteWindowRows` promised, which is exactly the overflow the window exists to prevent.
...
**The query runs before Ink mounts, and that is not stylistic.** Reading the reply needs a `data` listener on stdin, which switches it to flowing mode and drains chunks out from under Ink's read loop — the same hazard §3.9 documents for scroll input. After mount, stdin belongs to Ink and a late reply would be *typed into the prompt* as garbage. So [src/index.ts](../src/index.ts) fires the query right after the TTY preconditions and awaits it just before `render()`, with the plugin-loader await, the resume plan, and agent creation in between: the terminal's round trip overlaps work that was happening anyway and costs the boot nothing measurable. Raw mode is required (an unset terminal line-buffers the reply until Enter) and is restored afterward, as is the paused state. The `PROBE_TIMEOUT_MS` (100ms) deadline is the whole safety story — a terminal that never answers costs at most that. And the probe only runs at all when the setting is `auto`: an explicit `dark` or `light` never asks, because a probe nobody reads is not merely wasted — its `data` listener, still attached if the boot outran the deadline, would compete with Ink's reader for the same bytes.

**The boot window holds the keyboard.** The probe restores stdin to the mode it found, which at boot is canonical with echo on — and everything between that and Ink's mount would otherwise run that way. Keystrokes typed at a screen that looks ready were echoed by the tty at the cursor, above the frame Ink has not drawn yet (input appearing "before the box"), and the ones typed while the probe's listener was attached were swallowed into its buffer (input that never arrives). So the runner takes stdin raw and paused for the whole boot window, right after starting the probe: raw silences the echo, and a paused stream keeps the bytes queued in the terminal until Ink mounts and reads them — the first keystrokes land in the prompt instead of on the shell or in a probe buffer. A stdin that refuses raw mode degrades to the old behaviour rather than failing the boot, and a `finally` releases the hold on every path, including the ones that never reach Ink.
...
**No test can see the split, so a human has to.** chalk's color level is 0 under vitest, so not one frame the suite renders carries an SGR byte and the two weights are byte-identical to it. `hookTone` is pinned as a pure function instead, and `pnpm tty-check` prints a quiet, a blocking and an unrecognized row in the codes Ink resolves those colors to — the tone of each comes from the shipped `hookTone`, so what the human is asked to judge is the real thing (§3.4).
...
- **Sub-agent visualization.** Sub-agent outputs render as nested MessageList rows with a `↳` indent. **Not built, and — contrary to what this entry said for three revisions — not blocked either.** The claim was that `@deepseek-ai/dsh-subagent` and `@deepseek-ai/dsh-tool-workflow` exist only on `0.1.2-rc.x`, whose peer range would drag a second `dsh-session` into the tree and split the module augmentation that types session events. That is false: both publish **`0.1.0-rc.7`**, the line this package pins, and their peers there are `@deepseek-ai/dsh-session@^0.1.0-rc.7` throughout. `subagent/descriptor` is augmented into `SessionEventMap` by `dsh-subagent`'s own `descriptor.d.ts` at that version, and `tool-workflow/run-start|agent-start|agent-end|run-end` by `dsh-tool-workflow`'s `types.d.ts`.

  This is the **fourth** time this list wrote off a shipped capability as having no usable package — MCP twice, hooks once, and now this. Every one of the four was refuted by two commands: `npm view @deepseek-ai/<pkg> versions --json`, then reading the `.d.ts` in the tarball. Run them before writing "blocked" here again.

  What remains is the work itself, and one real design question §1.16 has now answered once: a sub-agent's rows are nested inside the entry that spawned them or they are not, and the `marginTop` and `tool/result`-pairing arguments that settled it for Code Mode apply here unchanged.
- **Code Mode sub-calls.** *Shipped.* Not on any list in this document before it shipped, and the blind spot was this package's own: `cordis.patch.yml` mounts `code-runtime`, so a `run_code` program's bridged tool calls were being logged in every assembly the TUI ships in and drawn in none of them. A sub-call is now one `↳` row inside its parent entry — see §1.16. It cost no new dependency: `dsh-tools` is already a peer and declares both payloads, so `src/types.ts` imports the augmentation instead of copying it.
