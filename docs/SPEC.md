...
**A settled resize repaints the screen.** Terminals may reflow rows already drawn while Ink still tracks logical rows, so cursor-relative erasure cannot be made reliable during a resize storm. The real TTY uses the alternate screen and one owner for resize: after 120ms of quiet, `index.ts` calls `instance.clear()`, clears the alternate screen, and calls `instance.rerender()` exactly once. The primary screen and shell scrollback are restored on exit. See [the resize lesson](lessons/resize-reflow.md).
