...
- **Banner / StatusBar** — the top zone. The session opens with the **Banner**: a two-column brand splash. The left column is the pixel-art whale, framed by a blank row above and below, with the slogan `探索未至之境！` centered under it. The right column is the block-letter `DEEPSEEK` / `HARNESS` wordmark. The four meta facts are **split across both columns**, not stacked in one: the left carries *where am I* (`<session id> · v<version>`, `<cwd> (<branch>*)`) and the right carries *what and how* (`provider/model`, the tip line). The split exists because both columns are the same height above the meta block — the slogan leaves the left column with empty rows while the right column would otherwise carry four lines alone. The banner is **static output** (Ink's `<Static>`): written to the terminal exactly once and never redrawn, so it scrolls away like any other past output and never costs the live frame a row. The compact **StatusBar** takes over as the live header as soon as there is a message to head, carrying the same identity in two rows plus the live token counts and run state. `/clear` empties the view and prints a fresh banner.
  ...
  **Three width tiers**, chosen by `bannerTier(columns)`:
  ...
  7. **The banner is written once, as static output, and no rendered line may ever exceed the terminal width.** These are the same rule seen from two sides. Ink erases the previous dynamic frame with `eraseLines(<logical line count>)`; a line wider than the terminal is wrapped by the terminal into two *physical* rows, so the count comes up short and the erase leaves the excess on screen. Ink also redraws the whole dynamic frame on every stdout `resize` — and it relays out the tree without re-rendering React, so the banner gets laid out at the new width while still holding the tier it chose at the old one. Nineteen rows of art in the dynamic frame plus a terminal that emitted a dozen resize events while starting up therefore stacked a dozen shredded banners down the screen. Two fixes, both needed: `<Static>` takes the banner out of the redraw path entirely, and shrinkable columns (rule 3) mean the one stale frame Ink writes between the resize and React's re-render clips at the right edge instead of crossing it. `tests/banner-frame.spec.ts` pins the invariant at nine widths, across a live shrink, and over a replay of the reported resize storm — twelve `resize` events used to write twelve banners, and now write one. "Wider than the terminal" includes *exactly as wide as* the terminal, which is why the live frame reserves the last column. And width arithmetic is only half of it: a terminal that rewraps rows it has already drawn breaks the erase at *any* width, so a settled resize repaints the screen. See the two notes under this list.
...
**A settled resize repaints the screen.** Reserving a column fixes the arithmetic Ink controls, and is not enough: it did not stop the ladder, and a real pty told us why. Driven through a programmatic drag, Ink's byte stream is *correct* — a four-logical-line frame, none of it filling the last column, erased with `eraseLines(4)`. The rows it counts on, however, are not the rows still on screen. iTerm2 and Terminal.app **reflow** when the window narrows: every row already drawn is rewrapped to the new width, so a frame that was written as four rows is now eight, Ink erases four of them, and the other four stay. This is width-independent — no margin can prevent it, because the damage is done to rows that were already correct when written. It also explains the exact debris the report showed: a *complete* `╭───╮` (copying rejoins a soft-wrapped row's two halves) above a content row with **no closing `│`** (that half is the one Ink erased). So `useResizeRepaint` listens for `resize`, waits 120 ms of quiet — comfortably past Ink's own 32 ms render throttle, and trailing-edge so the repaint lands after Ink's last resize render — and writes `ESC[2J ESC[H` through `useStdout().write`, which resets Ink's line count and lays the frame down again on a clean screen. Two consequences are deliberate. Debris is still visible *during* a drag; it is cleaned up when the drag stops, not continuously, because repainting per event would fight the storm rather than outlast it. And the banner is static output living *above* the frame, so clearing the screen clears it too: a resize ends with the session log and the prompt, no splash. `/clear` prints a fresh one. The escape is `2J`, never `3J` or the alternate screen buffer — the shell history above the app is the user's, and neither losing it nor restoring a pre-launch screen is what this app is for.
