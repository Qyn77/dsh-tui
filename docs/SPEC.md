...
`tests/frame-erase.spec.ts` pins this by replaying a real six-step drag through a small terminal emulator under **both** wrap readings plus a one-column lag: all three must end with one prompt box on screen, and the un-reserved version ends with eight. It also measures the StatusBar — the live frame's other framed child, and the one doing its own width arithmetic — from 120 columns down to 20.
