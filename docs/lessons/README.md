...
| [Message list scroll](message-list-scroll.md) | The mounted window was sliced by entry count while the box clipped by row, so the clipped edge was the bottom and the newest messages were unreachable; anchor with `column-reverse`, measure the viewport, and never attach a `data` listener to Ink's stdin. | terminal, ink, scroll, layout | Resolved |
