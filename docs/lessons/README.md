...
| [Ambiguous-width rows](ambiguous-width-smear.md) | East-Asian ambiguous glyphs (`·`, braille, `⏵`) count one column to Ink and two in CJK-configured terminals; a wrapped chrome row desyncs `eraseLines` and leaves a stable top-of-frame residue. Autowrap (`?7l`) stays off for the app's lifetime so a too-wide row clips instead of wrapping. | terminal, ink, width, cjk, erase | Resolved |
